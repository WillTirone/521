...
maytwenty = data_all %>% filter(day(datetimes) == 20 & month(datetimes) == 05) 
...
```{r}
pca_prcomp <- prcomp(data_all %>% select(-Tree, -Direc, -datetimes) %>% na.omit(), scale. = TRUE, center = TRUE)
...
```{r}
hist(data_all$humid_temp)

hist(data_all$humid_adj)

#hist(log(data_all$hamatop))

#hist(log(data_all$hamabot))
...
(a) Any suggestions for improving the first two plots in Figure 4? Can you distinguish all the colors in these two plots?
    ...
    unique(factor(data_all$Height)) #59 levels

    min(na.omit(data_all$Height)) #10.5

    max(na.omit(data_all$Height)) #66.5
