...
We decided not to plot result_time or epoch, as these are just time measurements. We also did not plot nodeid or parent since these are identifying keys for the specific nodes.

Voltage is not consistent between the two. data_log is in volts, while data_net appears to be 100x the amount of voltage. Since the sensors have a maximum range up to 10 volts, from consulting the spec sheet here ( https://sensirion.com/products/catalog/SHT11/, https://docs.rs-online.com/6fa1/0900766b809880e5.pdf), the original scale from 0 to 1000 is not correct. Further, consulting the orders of magnitude for voltage (https://en.wikipedia.org/wiki/Orders_of_magnitude\_(voltage)), we believe the measurement for voltage in data_net is actually in centivolts. Thus, we divided by 100 to maintain the same range.

Since the nodes were placed 0.1 - 1.0 m from the tree trunk, we divided the depths by 100 to scale them properly.
...
Here, we rescale depth and voltage in their respective dataframes and then concatenate into the corrected form of data_all with adjusted scales.

```{r}
data_net = data_net |> mutate(voltage = voltage/100,
                   depth = depth/10)
...
Our initial row count is 416,036, and removed 12,532 rows that had NA values for humidity, temperature, and both PAR measurements. Each row was missing data from every column.
