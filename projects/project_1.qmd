...
| Variable   | Notes                                  |
|------------|----------------------------------------|
| humidity   | less than 0 greater than 100           |
| Humid Temp |                                        |
| hamatop    | incident                               |
| hamabot    | reflected, remove max value, too large |

```{r}
max((data_all$parent)) # largest 16-bit integer, throw out 

max((data_all$depth)) # largest 8-bit 
...
```{r}
# checking for bad "characteristic" data like labels etc. 
unique(data_all[,'Direc'])
```

```{r}
# may want to fill in this data
data_all |> filter(is.na(Direc))
```
...
data_all = data_all |> filter(voltage >= 2.4 & voltage <= 5.5) |> # per spec sheet
