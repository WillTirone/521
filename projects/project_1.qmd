...
data_all = data_all |> filter(voltage >= 2.4 & voltage <= 5.5) |> 
...
```{r}
pca_prcomp <- prcomp(data_all_cleaned %>% select(-Tree, -Direc, -result_time) %>% na.omit(), scale. = TRUE, center = TRUE)
